...
def train_model_yfi():
    ...
    return model.save("my_model_yfi.h5")

def yfi_model_prediction():
    time_step = 15
    X_train, y_train = create_dataset(train_data, time_step)
    X_test, y_test = create_dataset(test_data, time_step)
    ...
    model = keras.models.load_model('my_model_yfi.h5')
...
def yfi_get_new_prediction():
    new_pred_plot = yfi_model_prediction()
    json_result = new_pred_plot.to_json(orient='records')
    addName = "["+json_result+","+"{\"name\": \"YFI\"}, {\"RMSE\":"+ yfi_get_rmse()+"},{\"R-Square\":"+ yfi_get_r2()+"},{\"MAPE\":"+ yfi_get_mape()+"}]"
    return addName
    #json_result = new_pred_plot.to_json()
    #name = [{name}]
    #addName = json_result.append[{"name": "yfi"}]
    #return addName