...
def train_model_paxg():
    ...
    return model.save("my_model_paxg.h5")


def paxg_model_prediction():
    model = keras.models.load_model('my_model_paxg.h5')
    time_step = 15
    X_train, y_train = create_dataset(train_data, time_step)
    X_test, y_test = create_dataset(test_data, time_step)
...
def paxg_get_new_prediction():
    new_pred_plot = paxg_model_prediction()
    json_result = new_pred_plot.to_json(orient='records')
    addName = "["+json_result+","+"{\"name\": \"PAXG\"}, {\"RMSE\":"+ paxg_get_rmse()+"},{\"R-Square\":"+ paxg_get_r2()+"},{\"MAPE\":"+ paxg_get_mape()+"}]"
    return addName
    #json_result = new_pred_plot.to_json()
    #name = [{name}]
    #addName = json_result.append[{"name": "paxg"}]
    #return addName